...
const resizeCover = document.querySelector('.resize-cover');
resizeCover.style.opacity = 0;
...
class ScrollTriangles {
	...
	setDim() {
		...
		this.triCols = (this.columns - 1) * 2;
		this.triRows = (this.rows - 1);
	}
	...
	makeSvg(callback) {
		...
		console.log("before child count: " + scrollElement.childElementCount);
		while (scrollElement.lastChild) {
			scrollElement.removeChild(scrollElement.lastChild);
		}
		console.log("after child count: " + scrollElement.childElementCount);
        for (let i = 0; i < this.triangles.length; i++) {
            this.triangles[i].createSvgTri();
        }
		console.log("new child count: " + scrollElement.childElementCount);
...
function render(anim) {
	console.assert(anim.animatables.length == triangles.triCols * triangles.triRows, 'row and column error');

	if (!canSvg) {
		ctx.clearRect(0, 0, scrollElement.width, scrollElement.height);
	}
	
	for (var row = 0; row < triangles.triRows * 2; row++) {
		var opacity = findOpacity(anim, row);
		if (row >= triangles.triRows * 2) {
			continue;
		}
		const r = Math.floor(row / 2) * triangles.triCols;
		var start;
		if (row % 4 == 0 || row % 4 == 1) {
			start = row % 2;
		}
		...
		for (var i = start; i < triangles.triCols; i += 2) {
			const tNum = r + i;
...
function whileResize() {
	resizeCover.style.opacity = 1;
	clearTimeout(timeout);
	timeout = setTimeout(function() { doneResize(); }, 100);
}
...
function doneResize() {
	...
	triangles.generate();
	if (canSvg) {
		triangles.makeSvg(function() { resizeCover.style.opacity = 0; });
	}
...
// for window resize
window.addEventListener('resize', whileResize);
...
// 			const tNum = r + i;
// 			// render a single triangle
// 			triangles.triangles[tNum].draw(.6);
// 		}
// 	}
// }
