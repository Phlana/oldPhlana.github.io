...
class ScrollTriangles {
	constructor() {
		...
		// const width = element.getAttribute('width') + this.size * 2;
		// const height = element.getAttribute('height') + this.size * 2;
...
function render(anim) {
	...
	for (var row = 0; row < triRows * 2; row++) {
		...
		for (var i = start; i < triCols; i += 2) {
			...
			if (canSvg) {
				anim.animatables[tNum].target.drawSvg(opacity);
			}
			else {
				anim.animatables[tNum].target.drawCanvas(opacity);
			}
		}
	}
}
...
window.addEventListener('scroll', () => {
	...
	logoAnimationEl.style.transform = `scale(${logoScale}, ${logoScale})`;
